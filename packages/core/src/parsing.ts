...
export const parseShouldRespondFromText = (
    text: string
): "RESPOND" | "IGNORE" | "STOP" | "RESPOND_ACP" | "SELF_RESPOND_ACP" | "RESPOND_PAYMENT_CONFIRMED" | null => {
    const match = text
        .split("\n")[0]
        .trim()
        .replace("[", "")
        .toUpperCase()
        .replace("]", "")
        .match(/^(RESPOND|IGNORE|STOP|RESPOND_ACP|SELF_RESPOND_ACP|RESPOND_PAYMENT_CONFIRMED)$/i);
    return match
        ? (match[0].toUpperCase() as "RESPOND" | "IGNORE" | "STOP" | "RESPOND_ACP" | "SELF_RESPOND_ACP" | "RESPOND_PAYMENT_CONFIRMED")
        ...
        ? "RESPOND_ACP"
        : text.includes("SELF_RESPOND_ACP")
        ? "SELF_RESPOND_ACP"
...
export const parseActionResponseFromText = (
    ...
    for (const line of lines) {
        ...
        if (trimmed === "[LIKE]") actions.like = false;
